...
public class BeeEntity extends AnimalEntity implements IAngerable, IFlyingAnimal {
   ...
   private boolean isFlowers(BlockPos pos) {
      return this.world.isBlockPresent(pos) && this.world.getBlockState(pos).getBlock().isIn(BlockTags.POLLINATABLE);
   }
   ...
   class PollinateGoal extends BeeEntity.PassiveGoal {
	   private final Predicate<BlockState> flowerPredicate = (p_226499_0_) -> {
		   if(p_226499_0_.isIn(BlockTags.POLLINATABLE)) {
			if(p_226499_0_.isIn(BlockTags.FLOWERS)) {
	         if (p_226499_0_.isIn(BlockTags.TALL_FLOWERS)) {
				...
				return (p_226499_0_.isIn(BlockTags.BUSHES) && p_226499_0_.getBlockState().get(HazelnutBush.AGE) == 4);
			}
	       }
		return false;
	      };
